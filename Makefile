...
release: clean set-build generate deps format
	@mkdir build || echo -e "$(WARN_COLOR)==> Build dir (/build) allready exist.$(NO_COLOR)"
	@echo -e "$(OK_COLOR)==> Building for linux 32 ...$(NO_COLOR)"
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=1 GOOS=linux GOARCH=386 go build -o build/${APP_NAME}-linux-386 -ldflags "$(LDFLAGS_RELEASE)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build -o build/${APP_NAME}-linux-amd64 -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=1 GOOS=linux GOARCH=arm GOARM=6 CC="armv6l-unknown-linux-gnueabihf-gcc" go build -o build/${APP_NAME}-linux-armv6 -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=0 GOOS=darwin GOARCH=386 go build -o build/${APP_NAME}-darwin-386 -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o build/${APP_NAME}-darwin-amd64 -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=1 GOOS=windows GOARCH=386 CC="i686-w64-mingw32-gcc" go build -o build/${APP_NAME}-win-386.exe -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
	...
	cd $(shell pwd)/.gopath/src/github.com/linuxisnotunix/Vulnerobot && GOPATH=$(shell pwd)/.gopath CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC="x86_64-w64-mingw32-gcc" go build -o build/${APP_NAME}-win-amd64.exe -ldflags "$(LDFLAGS_RELEASE)"
	@echo -e "$(OK_COLOR)==> Trying to compress binary ...$(NO_COLOR)"
...
update-deps: dev-deps
	...
	$(GOPATH)/bin/vendetta -n github.com/linuxisnotunix/Vulnerobot -u
#@go get -d -v -u ./...
